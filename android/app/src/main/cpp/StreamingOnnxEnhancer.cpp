...
void StreamingOnnxEnhancer::allocateTensors()
{
    ...
    for (const auto &s : mInputNamesStr)
        mInputNames.push_back(s.c_str());
    for (const auto &s : mOutputNamesStr)
...
void StreamingOnnxEnhancer::processHop(const float *hop_in, float *hop_out)
{
    ...
    // Linearize ring buffer into mWindowedFrame (oldest → newest)
    ...
    // Magnitude + phase: update mNoisyMag in-place (tensor already points here)
    ...
    // OLA (overlap-add with synthesis window)
    for (int i = 0; i < mWinLength; ++i)
    {
...
void StreamingOnnxEnhancer::prewarm()
{
    if (!mSession)
        return;

    std::vector<float> zeros(mHopLength, 0.0f);
    std::vector<float> dummyOut(mHopLength, 0.0f);
    processHop(zeros.data(), dummyOut.data());

    // Reset all stateful buffers so the engine starts from a clean slate.
    std::fill(mAnalysisBuffer.begin(), mAnalysisBuffer.end(), 0.0f);
    mAnalysisWriteIdx = 0;
    std::fill(mOlaSignal.begin(), mOlaSignal.end(), 0.0f);
    std::fill(mOlaNorm.begin(), mOlaNorm.end(), 0.0f);
    for (auto &s : mStateBacking)
    {
